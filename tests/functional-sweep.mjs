...
async function assertDirectEditor(page, width, height) {
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      wizardPanel: Boolean(
        document.querySelector('#wizardSection,.wizard-nav,#wizardNextBtn'),
      ),
      wizardGlobals: Boolean(
        window.kefeWizard || window.kefeEditingFlow,
      ),
      horizontalOverflow:
...
async function assertSections(page) {
  const sections = await page.evaluate(() => ({
    ...
    backgroundAccept:
      document.querySelector('#backgroundInput')?.accept || '',
  }));
  if (
    !sections.audio ||
    !sections.format ||
    ...
    throw new Error(
      `Missing core editor sections: ${JSON.stringify(sections)}`,
    );
  }
  if (
    !sections.audioAccept.includes('audio') ||
    !sections.backgroundAccept.includes('video')
  ) {
    throw new Error(
      `Media inputs are not configured for the core media flow: ${JSON.stringify(sections)}`,
    );
  }
}
...
async function seedLyrics(page) {
  await page.evaluate(() => {
    ...
    textarea.value =
      '[00:00.00]Functional test line\n[00:02.00]Second test line';
    textarea.dispatchEvent(new Event('input', { bubbles: true }));
    window.state.audio = {
      ...(window.state.audio || {}),
      ready: true,
      file: { name: 'functional-test.mp3' },
      duration: 4,
    };
  });
  await page.waitForFunction(
    () => document.getElementById('lyricsText')?.value.includes('Functional test line'),
    null,
    { timeout: 3000 },
  );
}
...
async function assertEditing(page) {
  ...
  if (!['9:16', '1:1', '16:9'].includes(format))
    throw new Error(`Unexpected default aspect ratio: ${format}`);

  await clickNav(page, 'text', 'textSection');
  await seedLyrics(page);
  await page.locator('#lyricStyleBlock [data-effect="brat"]').click();
  await page.waitForFunction(
    () => window.state?.style?.effect === 'brat',
    null,
    { timeout: 3000 },
  );
  ...
  await page
    .locator('#backgroundSection [data-background-preset="gradient"]')
    .click();
  await page.waitForFunction(
    () => window.state?.background?.type === 'image',
    null,
    { timeout: 3000 },
  );
  ...
  if (!Object.values(globals).every(Boolean))
    throw new Error(
      `Core runtime globals missing: ${JSON.stringify(globals)}`,
    );

  const analysis = await page.evaluate(() =>
    window.kefeAnalysis.analyzeLyrics(
      window.document.getElementById('lyricsText').value,
      4,
    ),
  );
  ...
  if (!renderPlan || !renderPlan.preset)
    throw new Error(`Smart Render did not prepare: ${JSON.stringify(renderPlan)}`);
...
async function runViewport(width, height) {
  ...
  page.on('pageerror', (error) =>
    errors.push(`${width}x${height} pageerror: ${error.message}`),
  );
  page.on('console', (message) => {
    if (message.type() === 'error')
      errors.push(`${width}x${height} console: ${message.text()}`);
  });
  try {
    await boot(page);
