...
(() => {
  ...
  function refreshHeaderWidget() {
    ...
    if (!badge) return;
    if (!currentUser) badge.textContent = 'Sign in';
    else if (entitlement.plan === 'pro') badge.textContent = 'Pro';
    else if (entitlement.plan === 'trial') badge.textContent = `Trial · ${entitlement.trialDaysLeft}d left`;
    else badge.textContent = 'Upgrade';
  ...
  function closeModal() { document.querySelectorAll('.kefe-auth-modal').forEach((m) => m.remove()); }
  ...
  function openAuthModal(mode = 'login') {
    ...
    overlay.querySelector('#authSwitch').addEventListener('click', (e) => { e.preventDefault(); openAuthModal(isLogin ? 'signup' : 'login'); });
    overlay.querySelector('#authForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        closeModal(); refreshHeaderWidget(); applyGating();
  ...
  function openAccountModal() {
    const planLine = entitlement.plan === 'pro' ? 'Pro — thanks for subscribing.' : entitlement.plan === 'trial' ? `Free trial — ${entitlement.trialDaysLeft} day${entitlement.trialDaysLeft === 1 ? '' : 's'} left.` : 'Free plan.';
    const overlay = openModal(`
      <div class="modal-header"><h3>Account</h3><button class="close-btn" data-close>×</button></div>
      <div class="modal-body"><p>${currentUser.email}</p><p class="auth-subtext">${planLine}</p>
      ...
      <button id="logoutBtn" class="full-width">Sign out</button></div>
    `, 'kefeAccountModal');
    overlay.querySelector('[data-close]').addEventListener('click', closeModal);
    overlay.querySelector('#logoutBtn').addEventListener('click', async () => {
      await api('/api/auth/logout', { method: 'POST' }); currentUser = null; entitlement = { plan: 'free', isPro: false, trialDaysLeft: null };
      closeModal(); refreshHeaderWidget(); applyGating(); toast('Signed out');
    });
    ...
    if (manageBtn) manageBtn.addEventListener('click', async () => { try { const data = await api('/api/billing/create-portal-session', { method: 'POST' }); window.location.href = data.url; } catch (err) { toast(err.message); } });
  }

  function openUpgradeModal() {
    const overlay = openModal(`
      <div class="modal-header"><h3>Upgrade to Pro</h3><button class="close-btn" data-close>×</button></div>
      <div class="modal-body"><ul class="upgrade-list"><li>All lyric effects — Eternal, Aurora, Instagram Lyrics, Fade Up</li><li>1080p export + Instagram/TikTok presets</li><li>Priority support for new effects</li></ul><button id="startCheckout" class="primary full-width">Continue to checkout</button></div>
    `, 'kefeUpgradeModal');
    overlay.querySelector('[data-close]').addEventListener('click', closeModal);
    overlay.querySelector('#startCheckout').addEventListener('click', async () => {
      if (!currentUser) { closeModal(); return openAuthModal('signup'); }
      try { const data = await api('/api/billing/create-checkout-session', { method: 'POST' }); window.location.href = data.url; } catch (err) { toast(err.message); }
    });
  }
  ...
  function applyGating() {
    document.querySelectorAll('[data-effect]').forEach((btn) => {
      ...
      if (locked && !btn.querySelector('.lock-badge')) { const badge = document.createElement('span'); badge.className = 'lock-badge'; badge.textContent = '🔒'; btn.appendChild(badge); }
      else if (!locked) { const badge = btn.querySelector('.lock-badge'); if (badge) badge.remove(); }
    });
    const presetSelect = document.getElementById('exportPreset');
    if (presetSelect) Array.from(presetSelect.options).forEach((opt) => { const locked = PREMIUM_EXPORT_PRESETS.includes(opt.value) && !entitlement.isPro; if (locked && !opt.textContent.includes('Pro')) opt.textContent += ' (Pro)'; if (!locked) opt.textContent = opt.textContent.replace(' (Pro)', ''); });
  }

  document.addEventListener('click', (e) => { const btn = e.target.closest('[data-effect]'); if (btn && btn.classList.contains('locked-premium')) { e.preventDefault(); e.stopImmediatePropagation(); openUpgradeModal(); } }, true);
  document.addEventListener('change', (e) => { if (e.target && e.target.id === 'exportPreset') { const opt = e.target.selectedOptions[0]; if (opt && PREMIUM_EXPORT_PRESETS.includes(opt.value) && !entitlement.isPro) { e.stopImmediatePropagation(); e.target.value = '720p'; openUpgradeModal(); } } }, true);

  async function boot() {
    buildHeaderWidget();
    try { const data = await api('/api/auth/me'); if (data.user) { currentUser = data.user; entitlement = { plan: data.user.plan, isPro: data.user.isPro, trialDaysLeft: data.user.trialDaysLeft }; } } catch (err) {}
    refreshHeaderWidget(); applyGating();
    const params = new URLSearchParams(window.location.search);
    if (params.get('checkout') === 'success') {
      toast('Payment received — Pro features unlocking shortly.');
      setTimeout(async () => { try { const data = await api('/api/auth/me'); if (data.user) { currentUser = data.user; entitlement = { plan: data.user.plan, isPro: data.user.isPro, trialDaysLeft: data.user.trialDaysLeft }; refreshHeaderWidget(); applyGating(); } } catch (e) {} }, 2500);
      history.replaceState({}, '', window.location.pathname);
    } else if (params.get('checkout') === 'cancelled') history.replaceState({}, '', window.location.pathname);
  }
