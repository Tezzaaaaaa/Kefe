...
(() => {
    ...
    function setProgress(pct) {
        ...
        // Remember the fraction so the live timer can estimate remaining time
        // whenever the transcription engine reports meaningful progress.
        const f = Math.max(0, Math.min(1, pct / 100));
        if (f > progressFraction + 0.005) { progressFraction = f; progressSamples++; }
    }

    /* ---------- real-time generation timer (elapsed + estimated remaining) ---------- */
    ...
    function renderTimer() {
        ...
        // Only quote a remaining time when the engine reports steadily increasing
        // progress — never fake precision.
    ...
    /* ---------- source resolution (uses the app's existing master source) ---------- */
    ...
    function normaliseTimedSegments(list) {
        return (list || [])
            .map(s => ({
                ...
                time: Math.max(0, Number(s?.start) || 0),
                endTime: Number.isFinite(Number(s?.end)) ? Number(s.end) : (Number(s?.start) || 0) + 3
            }))
            .filter(s => s.text)
            .sort((a, b) => a.time - b.time);
    }
    /* ---------- provider: local Whisper in the browser (transformers.js) ---------- */
    registry.register({
        id: 'local-whisper',
        ...
        async transcribe(ctx) {
            ...
            let mod;
            try {
                mod = await import('https://cdn.jsdelivr.net/npm/@xenova/transformers@2.17.2/dist/transformers.min.js');
            } catch (e) {
                throw new Error('Could not load the local speech model (an internet connection is needed the first time). Try the KEFE server provider instead.');
            }
            try { mod.env.allowLocalModels = false; } catch (e) { /* older builds */ }
            const modelId = options?.model || 'Xenova/whisper-tiny.en';
            const asr = await mod.pipeline('automatic-speech-recognition', modelId, {
                quantized: true,
                progress_callback: p => {
                    if (p && p.status === 'progress' && Number.isFinite(p.progress)) onProgress(Math.min(90, 3 + p.progress * 0.85));
                }
            });
            ...
            const output = await asr(audio.float32, {
                ...
                chunk_callback: () => {
                    chunksDone++;
                    onProgress(Math.min(96, 5 + (chunksDone / estimatedChunks) * 91));
                }
            });
    ...
    /* ---------- provider: KEFE server endpoint (key stays server-side) ---------- */
    registry.register({
        id: 'server',
        ...
        async transcribe(ctx) {
            ...
            if (Array.isArray(data?.words) && data.words.length) {
                const words = data.words
                    .map(w => ({ text: String(w.word ?? w.text ?? '').trim(), start: Number(w.start), end: Number(w.end ?? (Number(w.start) + Number(w.duration || 0))) }))
                    .filter(w => w.text && Number.isFinite(w.start) && w.start >= 0);
                if (words.length) return { words };
            }
            if (Array.isArray(data?.segments) && data.segments.length) {
                const segments = data.segments
                    .map(s => ({ text: String(s.text || '').trim(), start: Number(s.start), end: Number(s.end) }))
    ...
    /* ---------- UI: dedicated sections (inserted after Lyrics & Captions) ---------- */
    ...
    genSection.innerHTML = [
        ...
        '<p class="caption-gen-lede">Transcribes the speech in your loaded audio or video into timed captions, then renders them with KEFE\'s dedicated caption style — a conventional subtitle look, separate from the lyric effects.</p>',
        '<div class="control-row compact"><label for="captionGenProvider">Transcription engine</label>',
        '<select id="captionGenProvider"></select></div>',
        '<div class="control-row compact" id="captionModelRow"><label for="captionGenModel">Local model</label>',
        '<select id="captionGenModel"><option value="Xenova/whisper-tiny.en">Whisper Tiny · English · fast</option><option value="Xenova/whisper-base.en">Whisper Base · English · more accurate</option></select></div>',
    ...
    reviewSection.innerHTML = [
        ...
        '<button type="button" id="captionRegenBtn">Regenerate</button>',
        '<button type="button" id="captionOpenEditorBtn">Edit in Captions Editor</button>',
        '</div>',
        '<div class="status">Click a timestamp to jump the preview there. Fine timing edits live in the captions editor.</div>'
    ].join('');
    ...
    /* ---------- apply generated lines through the EXISTING caption system ---------- */
    function applyGeneratedCaptions(lines) {
        ...
        if (cs) {
            cs.textContent = `Generated ${state.captions.lines.length} caption segments from speech — review them in Caption Review, or open Edit Captions.`;
            cs.className = 'status success';
        }
    }
    /* ---------- caption review rendering (edits write straight into state) ---------- */
    function refreshReview() {
        const summary = $('captionReviewSummary');
        const rowsBox = $('captionReviewRows');
        if (!summary || !rowsBox) return;
        const lines = state.captions.lines || [];
        if (!lines.length) {
            summary.textContent = 'No captions yet — generate them in the Caption Generator.';
            summary.className = 'status';
            rowsBox.replaceChildren();
            return;
        }
        const wordCount = lines.reduce((n, l) => n + (Array.isArray(l.words) ? l.words.length : String(l.text || '').split(/\s+/).filter(Boolean).length), 0);
        summary.textContent = `${lines.length} caption segments · ~${wordCount} words · timings preserved from the transcript.`;
        summary.className = 'status success';
        rowsBox.replaceChildren(...lines.map((line, i) => {
            const row = document.createElement('div');
            row.className = 'caption-review-row';
            const time = document.createElement('button');
            time.type = 'button';
            time.className = 'caption-review-time';
            time.textContent = fmtShort(line.time);
            time.title = 'Jump the preview to this moment';
            time.addEventListener('click', () => {
                if (typeof window.setMasterTime === 'function') window.setMasterTime(Number(line.time) || 0);
                if (typeof window.redrawCurrentPreviewFrame === 'function') window.redrawCurrentPreviewFrame();
            });
            ...
            input.placeholder = 'What is said in this segment…';
            input.addEventListener('input', () => {
                if (!state.captions.lines[i]) return;
                state.captions.lines[i].text = input.value;
                if (typeof window.readiness === 'function') window.readiness();
                if (typeof window.redrawCurrentPreviewFrame === 'function') window.redrawCurrentPreviewFrame();
            });
    ...
    async function runGeneration() {
        if (running) return;
        if (window.isExporting) { setStatus('Finish or cancel the current export first.', 'error'); return; }
        const file = resolveSourceFile();
        if (!file) { setStatus('Load an audio or video source first (Back to Source).', 'error'); return; }
        const provider = registry.get(providerSelect?.value);
        if (!provider) { setStatus('No transcription engine available.', 'error'); return; }
        running = true;
        const genBtn = $('captionGenBtn'), regenBtn = $('captionRegenBtn');
        if (genBtn) genBtn.disabled = true;
        if (regenBtn) regenBtn.disabled = true;
        startCaptionTimer();
        setProgress(1);
        setStatus(`Decoding audio from ${file.name || 'the loaded source'}…`);
        try {
            ...
            if (!lines.length) throw new Error('No timed speech was found in this source.');
            applyGeneratedCaptions(lines);
            refreshReview();
            setProgress(100);
            setStatus(`Generated ${lines.length} caption segments with ${provider.label}. Review them in the next step.`, 'success');
            if (typeof window.toast === 'function') window.toast(`Generated ${lines.length} captions`, 'success');
        } catch (err) {
            setProgress(null);
            setStatus(err?.message || 'Caption generation failed.', 'error');
        } finally {
            running = false;
            stopCaptionTimer();
    ...
    function syncGenerateButton() {
        ...
        if (btn) btn.textContent = state.captions.lines?.length ? 'Regenerate Captions' : 'Generate Captions';
    }

    $('captionGenBtn').addEventListener('click', runGeneration);
    $('captionRegenBtn').addEventListener('click', runGeneration);
    $('captionOpenEditorBtn').addEventListener('click', () => { $('editCaptionsBtn')?.click(); });
