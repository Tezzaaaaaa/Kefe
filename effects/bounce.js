...
(() => {
  ...
  window.kefeEffects.bounce=function(ctx,w,h,style,lines,time){
    ...
    const c=u.contract('bounce'), size=Math.max(c.min,Math.min(c.max,Number(style.fontSize)||80)); u.setContractFont(ctx,'bounce',size);
    ...
    for(let i=0;i<words.length;i++){
      ...
      const lift=(1-e)*size*.55 + Math.max(0,b)*size*.14;
      const scale=.92+.08*e;
