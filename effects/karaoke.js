...
(() => {
  ...
  window.kefeEffects.karaoke = function(ctx,w,h,style,lines,time){
    ...
    const c=u.contract('karaoke'); const size=Math.max(c.min,Math.min(c.max,Number(style.fontSize)||82));
    u.setContractFont(ctx,'karaoke',size);
    ...
    for(let i=0;i<words.length;i++){
      ...
      ctx.save(); ctx.globalAlpha=.28+.72*clamp(reveal); ctx.fillText(word.text,x,y); ctx.restore();
      if(glow>0){
        ctx.save(); ctx.globalAlpha=.98*glow; ctx.fillStyle=style.accentColor||'#FFF'; ctx.shadowColor=ctx.fillStyle; ctx.shadowBlur=size*.08*glow; ctx.fillText(word.text,x,y); ctx.restore();
      }
      x+=widths[i]+gap;
    }
    ctx.restore();
