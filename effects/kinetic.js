...
(() => {
  ...
  function wrap(ctx,words,size,max,gap){
    u.setContractFont(ctx,'mixedmedia',size);const rows=[];let row=[],width=0;
    for(const word of words){const ww=ctx.measureText(word.text).width;const proposed=width+(row.length?gap:0)+ww;if(row.length&&proposed>max){rows.push(row);row=[];width=0;}row.push({...word,width:ww});width+=(row.length>1?gap:0)+ww;}
    if(row.length)rows.push(row);return rows;
  }
  function layout(ctx,words,size,max,gap,mode,seed){
    let rows=wrap(ctx,words,size,max,gap);
    if(rows.length>3&&size>42)rows=wrap(ctx,words,size*.86,max,gap*.9);
    const rowGap=size*.10,flat=rows.flat(),ranked=[...flat].sort((a,b)=>wordScore(b)-wordScore(a)),hero=ranked[0]?.text;
    if(mode==='stack'||(mode==='auto'&&rows.length===1&&flat.length>=5))rows=[flat];
    return rows.map((row,r)=>{
      const total=row.reduce((n,x)=>n+x.width,0)+Math.max(0,row.length-1)*gap;let x=-total/2;
      return row.map((word,i)=>{
        const jitter=(hash(seed+':'+r+':'+i)-.5)*size*.014;
        const centered=x+word.width/2+jitter;let xx=centered,yy=(r-(rows.length-1)/2)*(size+rowGap);
        if(mode==='split'||(mode==='auto'&&rows.length===1&&flat.length<=3)){
          const side=i%2?-1:1;xx=centered+side*size*(.11+.06*(i===0));yy+=Math.abs(i-row.length/2)*size*.025;
        }
        x+=word.width+gap;
        return {...word,x:xx,y:yy,hero:word.text===hero,index:i,row:r};
      });
    });
  }
  function drawWord(ctx,word,time,params,seed){
    ...
    const n=hash(seed+':'+word.text+':'+word.index),dir=n>.66?1:n<.33?-1:0;
    const travel=params.travel*(.65+.7*n),fromX=dir*travel*(1-enter),fromY=(n-.5)*params.travelY*(1-enter);
    const heroBoost=word.hero?1+params.hierarchy*.15:1-params.density*.045,activeBoost=1+params.hierarchy*.09*active;
    const settle=1+.028*params.energy*Math.sin(Math.PI*clamp((p.raw-.10)/.68));
    const scale=(.86+.14*enter)*settle*heroBoost*activeBoost,rotation=(n-.5)*params.rotation*(1-enter),alpha=enter*(1-.42*exit);
  ...
  window.kefeEffects.kinetic=function(ctx,w,h,style,lines,time){
    ...
    const mode=['auto','center','split','stack'].includes(style.kineticComposition)?style.kineticComposition:'auto';
    const sizeBase=Number(style.fontSize)||82,maxWidth=w*(.80+.08*(1-density)),gap=sizeBase*(.055+.035*density);
    const size=fit(ctx,text,sizeBase*(.96+energy*.12),maxWidth),rows=layout(ctx,words,size,maxWidth,gap,mode,text+'|'+a.index);
    const lp=u.lineProgress(a.line,time,.10,.20),enter=easeOut(lp.enter),exit=easeInOut(lp.exit),lineY=h*(.50+.025*(hash(text)-.5)),drift=(1-enter)*w*.035*(.5+motion);
