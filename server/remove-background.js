  ...
  async (req, res) => {
    ...
    if (!apiKey) {
      return res.status(501).json({
        error:
          'Background removal is not configured. Set REMOVE_BG_API_KEY on the KEFE server.',
      });
    }
    ...
    if (req.body.length > MAX_IMAGE_BYTES) {
      return res
        .status(413)
        .json({ error: 'Image is too large. Maximum size is 22 MB.' });
    }
    ...
    try {
      ...
      return res
        .status(502)
        .json({ error: 'The background-removal request failed upstream.' });
    }
  },
);
