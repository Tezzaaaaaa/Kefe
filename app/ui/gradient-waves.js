/* KEFE — Gradient Waves
   ...
   No framework dependency; exposes a reusable canvas renderer for both the
   landing surface and the editor background system. */
(() => {
  'use strict';
  ...
  function createGradientWaves(options = {}) {
    ...
    const horizon = hex(settings.horizonColor);
    const wave = hex(settings.waveColor);
    const crest = hex(settings.crestColor);
    let raf = 0;
    let running = true;
    let width = 1;
    let height = 1;
    ...
    const draw = now => {
      ...
      const horizonY = height * (0.34 + settings.waveRatio * 0.22);
      const zoom = Math.max(0.15, settings.zoom);
      const scale = Math.max(0.04, settings.waveScale);
      const amp = settings.amplitude * 0.018;
      const turb = settings.turbulence / 100;
      const bright = settings.brightness;
      const depth = Math.max(1, settings.fogDepth);
      const detail = settings.detail === 'high' ? 3 : settings.detail === 'medium' ? 2 : 1;
      const horizonFade = Math.max(0.01, settings.height);

      for (let y = 0; y < height; y++) {
        const ny = y / height;
        const dy = (y - horizonY) / height;
        for (let x = 0; x < width; x++) {
          const nx = x / width;
          const sx = (nx - 0.5) * zoom;
          const sy = dy * zoom * settings.tilt;
          ...
          const horizonMix = clamp(1 - Math.abs(dy) * depth * 0.011, 0, 1);
