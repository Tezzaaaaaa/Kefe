...
if(!runtime){window.addEventListener('load',()=>{const s=document.createElement('script');s.src='./app/ui/gradient-waves.js?v=20260903-5';s.async=false;document.body.appendChild(s);const bridge=document.createElement('script');bridge.textContent='window.state = state;';document.body.appendChild(bridge);const runtimeScript=document.createElement('script');runtimeScript.src='./app/ui/background-presets.js?v=20260903-5&runtime=1';runtimeScript.async=false;document.body.appendChild(runtimeScript)},{once:true});return}

/* Landing home: Gradient Waves is intentionally page-only and never added to the editor background choices. */
if(!document.querySelector('.kefe-landing-home')&&window.KefeGradientWaves){
  ...
  const waves=window.KefeGradientWaves.create({...window.KefeGradientWaves.defaults,opacity:.42,brightness:1.15,speed:.11});
  waves.canvas.classList.add('kefe-landing-waves');
  landing.appendChild(waves.canvas);
  ...
  const enter=()=>{document.body.classList.add('kefe-editor-entered');landing.classList.add('is-leaving');setTimeout(()=>{waves.destroy();landing.remove()},450)};
  ...
  window.kefeLandingEnter=enter;window.kefeLandingWaves=waves;
}
