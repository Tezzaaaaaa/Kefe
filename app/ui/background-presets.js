...
if(!runtime){window.addEventListener('load',()=>{const s=document.createElement('script');s.src='./app/ui/web-threads.js?v=20260905-1';s.async=false;document.body.appendChild(s);const bridge=document.createElement('script');bridge.textContent='window.state = state;';document.body.appendChild(bridge);const runtimeScript=document.createElement('script');runtimeScript.src='./app/ui/background-presets.js?v=20260905-1&runtime=1';runtimeScript.async=false;document.body.appendChild(runtimeScript)},{once:true});return}
if(!document.querySelector('.kefe-landing-home')&&window.KefeWebThreads){
  document.body.classList.add('kefe-has-landing');
  const landing=document.createElement('section');landing.className='kefe-landing-home';landing.setAttribute('aria-label','KEFE Visualiser home');
  const prism=window.KefeWebThreads.create({...window.KefeWebThreads.defaults});prism.canvas.classList.add('kefe-landing-threads');landing.appendChild(prism.canvas);document.body.prepend(landing);prism.refresh();
  ...
  const hero=document.createElement('div');hero.className='kefe-landing-hero';hero.innerHTML='<img src="./assets/branding/kefe-logo-light.svg" class="kefe-landing-logo" alt="KEFE Visualiser"><p>Bring your content to life with lyrics, captions, and visualisers.</p><button type="button" class="kefe-landing-start">Get started</button>';landing.appendChild(hero);
  requestAnimationFrame(()=>prism.refresh());
  const enter=()=>{document.body.classList.add('kefe-editor-entered');landing.classList.add('is-leaving');setTimeout(()=>{prism.destroy();landing.remove()},450)};
  landing.querySelector('.kefe-landing-start').addEventListener('click',enter);landing.querySelector('.kefe-landing-signin').addEventListener('click',()=>document.getElementById('accountBtn')?.click());
  window.kefeLandingEnter=enter;window.kefeLandingPrism=prism;
}
