...
(() => {
  ...
  const lyricEffectCopy = {
    ...
    storyfade: 'Cinematic line-by-line fade', decrypt: 'Scrambled characters resolve', blur: 'Blurred words sharpen into focus', shiny: 'Diagonal light sweep',
    scrolllines: 'Editorial multi-line horizontal motion', vhsglitch: 'Red VHS glitch with scanline tears', rise: 'Soft upward lift', slide: 'Smooth lateral glide',
    drop: 'Controlled downward arrival', drift: 'Gentle diagonal float'
  };
  ...
  function audioReady() {
    ...
    return Boolean(audio && (audio.file || audio.ready || Number(audio.duration) > 0 || media?.videoFile));
  }
  ...
  function ensureRenderer(src, key, dataAttr) {
    if (window.kefeEffects?.[key] || document.querySelector(`script[${dataAttr}]`)) return;
  ...
  function buildLyricEffects() {
    ...
    for (const name of ['scrolllines', 'rise', 'slide', 'drop', 'drift', 'storyfade']) {
      ...
      button.textContent = name === 'storyfade' ? 'Story Fade' : name[0].toUpperCase() + name.slice(1);
      button.title = lyricEffectCopy[name];
      button.addEventListener('click', () => window.setEffect?.(name));
    ...
    if (grid.dataset.names !== names.join('|')) {
      ...
      usable.forEach(sourceButton => {
        ...
        hint.textContent = lyricEffectCopy[name] || 'Lyric animation';
  ...
  function render() {
    ...
    if (step.id === 'intro') {
      ...
      panel.innerHTML = '<p class="wizard-panel-kicker">01 · Media</p><h3 class="wizard-panel-title">Add your media</h3><p class="wizard-panel-hint">Upload the audio or video that will drive the project. Song details appear with the media when available.</p>';
    } else if (step.id === 'lyrics') {
      panel.innerHTML = '<p class="wizard-panel-kicker">02 · Lyrics</p><h3 class="wizard-panel-title">Add and sync your lyrics</h3><p class="wizard-panel-hint">Use song details to find synced lyrics automatically, upload an LRC file, or edit and time the lyrics manually.</p>';
    } else if (step.id === 'style') {
      panel.innerHTML = '<p class="wizard-panel-kicker">03 · Style</p><h3 class="wizard-panel-title">Choose your lyric style</h3><p class="wizard-panel-hint">Pick the lyric treatment used by the live project preview.</p><div id="wizardStyleMount" class="wizard-style-mount"></div>';
    } else if (step.id === 'background') {
      panel.innerHTML = '<p class="wizard-panel-kicker">04 · Background</p><h3 class="wizard-panel-title">Choose your background</h3><p class="wizard-panel-hint">Use a preset or your own image/video. Background settings stay separate from lyric style.</p>';
    } else if (step.id === 'preview') {
      panel.innerHTML = '<p class="wizard-panel-kicker">05 · Preview</p><h3 class="wizard-panel-title">Preview the finished project</h3><p class="wizard-panel-hint">Play through the actual project before exporting.</p>';
    } else if (step.id === 'export') {
      panel.innerHTML = '<p class="wizard-panel-kicker">06 · Export</p><h3 class="wizard-panel-title">Export your lyric video</h3><p class="wizard-panel-hint">Export the same project shown in Preview.</p>';
    }

    $('wizardProgress').textContent = `${String(index + 1).padStart(2, '0')} / 06`;
  ...
  document.addEventListener('change', event => {
    if (steps[index].id === 'source' || steps[index].id === 'lyrics') syncNextButton();
