(() => {
  ...
  function update() {
    ...
    const lyrics =
      Array.isArray(window.state.lyrics?.lines) && window.state.lyrics.lines.length > 0;
    const background = Boolean(
      window.state.background?.image ||
      window.state.background?.video ||
      window.state.background?.type,
    );
    ...
    const setStat = (id, ready, pending) => {
      const node = $(id);
      if (!node) return;
      node.textContent = ready ? 'Ready' : pending;
      node.parentElement?.setAttribute('data-state', ready ? 'complete' : 'pending');
    };
    ...
    card.querySelector('.kefe-aid-card').dataset.state =
      complete === states.length ? 'complete' : 'pending';
    card.querySelectorAll('[data-kefe-flow]').forEach((item, index) => {
  ...
  function boot() {
    ...
    toggle?.addEventListener('change', () => {
      const details = $('kefeAidDetails');
      if (details) details.dataset.open = String(toggle.checked);
    });
