...
(() => {
  ...
  function create(options={}){const s={...D,...options},c=document.createElement('canvas');c.className='kefe-web-threads';c.setAttribute('aria-hidden','true');const x=c.getContext('2d');let w=1,h=1,d=1,raf=0,run=true,start=performance.now(),mx=.5,my=.5,tx=.5,ty=.5;const a=rgb(s.color1),b=rgb(s.color2),z=rgb(s.color3);
    const resize=()=>{const r=c.getBoundingClientRect();w=Math.max(1,Math.round(r.width));h=Math.max(1,Math.round(r.height));d=Math.min(devicePixelRatio||1,1.5);c.width=w*d;c.height=h*d;x.setTransform(d,0,0,d,0,0)};
    const move=e=>{const r=c.getBoundingClientRect();tx=clamp((e.clientX-r.left)/r.width,0,1);ty=clamp((e.clientY-r.top)/r.height,0,1)};
    ...
    resize();window.addEventListener('resize',resize,{passive:true});window.addEventListener('pointermove',move,{passive:true});raf=requestAnimationFrame(draw);return{canvas,stop(){run=false;cancelAnimationFrame(raf)},destroy(){run=false;cancelAnimationFrame(raf);window.removeEventListener('resize',resize);window.removeEventListener('pointermove',move);c.remove()}};
  }
