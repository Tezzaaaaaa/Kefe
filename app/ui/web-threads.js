...
(() => {
  ...
  function create(options={}){const s={...D,...options},c=document.createElement('canvas');c.className='kefe-web-threads';c.setAttribute('aria-hidden','true');const x=c.getContext('2d');let w=1,h=1,d=1,raf=0,run=true,start=performance.now(),mx=.5,my=.5,tx=.5,ty=.5;
    ...
    const move=e=>{const r=c.getBoundingClientRect();if(!r.width||!r.height)return;tx=clamp((e.clientX-r.left)/r.width,0,1);ty=clamp((e.clientY-r.top)/r.height,0,1)};
    ...
    return{canvas,refresh:resize,start(){if(!run){run=true;start=performance.now();raf=requestAnimationFrame(draw)}},stop(){run=false;cancelAnimationFrame(raf)},destroy(){run=false;cancelAnimationFrame(raf);ro?.disconnect();window.removeEventListener('resize',resize);window.removeEventListener('pointermove',move);c.remove()}};
  }
