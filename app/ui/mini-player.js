...
.kip-screen.is-vis .kip-art-wrap{opacity:0;transition:opacity .25s ease}
...
.kip-art-wrap{width:76px;height:76px;border-radius:5px!important;overflow:hidden;background:#12202f;box-shadow:0 4px 10px rgba(0,0,0,.55);transition:opacity .25s ease}
...
.kip-icon-btn{width:26px;height:26px;display:grid;place-items:center;padding:0;border:0;background:transparent;color:var(--kip-icon);cursor:pointer;border-radius:50%!important}
...
.kip-progress{position:relative;z-index:2;height:2.5px;border-radius:99px!important;background:rgba(255,255,255,.16);overflow:hidden}
...
.kip-wheel{position:relative;width:78%;aspect-ratio:1/1;margin:8px auto 0;border-radius:50%!important;background:radial-gradient(circle at 50% 40%,#14171d 0%,#0b0d11 68%,#05070a 100%);box-shadow:0 0 0 1px rgba(255,255,255,.04) inset,0 0 0 2px rgba(0,0,0,.5),0 10px 24px rgba(0,0,0,.65) inset,0 1px 0 rgba(255,255,255,.08);flex:0 0 auto}
...
.kip-wheel-btn{position:absolute;display:grid;place-items:center;padding:0;border:0;background:transparent;color:#b9c0cc;cursor:pointer;width:40px;height:40px;border-radius:50%!important;z-index:2}
...
.kip-close{position:absolute;top:14px;right:14px;width:26px;height:26px;display:grid;place-items:center;border:0;border-radius:50%!important;background:rgba(0,0,0,.4);color:#fff;cursor:pointer;z-index:5;opacity:.7}
.kip-close svg{width:12px;height:12px;fill:none;stroke:currentColor;stroke-width:2.2;stroke-linecap:round}
.kip-presets{position:absolute;inset:auto 12px 12px;max-height:66%;overflow:auto;background:rgba(8,12,20,.97);border-radius:14px!important;padding:10px;box-shadow:0 20px 50px rgba(0,0,0,.7);z-index:6}
...
`;
  ...
  player.innerHTML = `
    <div class="kip-shell">
      <button type="button" class="kip-close" id="kipClose" aria-label="Close"><svg viewBox="0 0 24 24"><path d="M6 6l12 12M18 6L6 18"/></svg></button>
      <div class="kip-screen" id="kipScreen">
        <canvas class="kip-canvas" id="kipCanvas" width="720" height="504"></canvas>
        <div class="kip-statusbar">
          <span class="kip-time" id="kipClock">9:41 AM</span>
          <span class="kip-mode" id="kipMode">Music</span>
        ...
        <div class="kip-main">
          ...
          <div class="kip-meta">
            <div class="kip-title-row"><span class="kip-title" id="kipTitle">Add music to begin</span></div>
        ...
        <div class="kip-dolby-row">
          ...
          <div style="display:flex;gap:2px">
            <button type="button" class="kip-icon-btn" id="kipUpload" aria-label="Upload"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 16V5M8 9l4-4 4 4M5 19h14"/></svg></button>
            <button type="button" class="kip-icon-btn" id="kipStar" aria-label="Favorite"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 3.5l2.6 6 6.4.5-4.9 4.2 1.5 6.3L12 17l-5.6 3.5 1.5-6.3L3 10l6.4-.5z"/></svg></button>
            <button type="button" class="kip-icon-btn" id="kipMore" aria-label="More"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><circle cx="6" cy="12" r="2.2"/><circle cx="12" cy="12" r="2.2"/><circle cx="18" cy="12" r="2.2"/></svg></button>
          </div>
        </div>
        ...
        <div class="kip-bottom-row">
          ...
          <button type="button" class="kip-icon-btn" id="kipMenu2" aria-label="Presets"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"><path d="M4 7h16M4 12h16M4 17h16"/></svg></button>
        </div>
      </div>
      <div class="kip-wheel">
        ...
        <button type="button" class="kip-wheel-btn kip-wheel-play" id="kipPlay" aria-label="Play"><svg viewBox="0 0 24 24"><path d="M8 5l11 7-11 7z"/></svg></button>
        <button type="button" class="kip-wheel-center" id="kipCenter" aria-label="Play"></button>
      </div>
      <input id="kipFiles" type="file" accept="audio/*" multiple hidden>
  ...
  function readMetadata(file){
    ...
    const title = parts.pop()?.trim() || base;
  ...
  function syncMeta(){
    ...
    if (t) {
      $('kipTitle').textContent = t.title;
      $('kipArtist').textContent = t.artist;
      $('kipAlbum').textContent = t.album || '';
    } else {
      $('kipTitle').textContent =text 'Add music to begin';
      $('kipArtist').Content = 'Nothing queued';
      $('kipAlbum').textContent = '';
    }
    try {
  ...
  function drawFrame(){
    ...
    if (player.classList.contains('is-hidden') || displayMode !== 'visualiser') return;
    resizeCanvas();
    ...
    try {
      ...
      if (style === 'butterchurn') {
        ...
        window.kefeButterchurn?.drawMini?.(ctx, canvas.width, canvas.height, audio.currentTime || 0, window.state, audio);
  ...
  function setDisplayMode(mode){
    displayMode = mode;
    $('kipScreen').classList.toggle('is-vis', mode === 'visualiser');
    $('kipMode').textContent = mode === 'visualiser' ? 'Visualiser' : 'Music';
    if (mode === 'visualiser') {
      resizeCanvas();
  ...
  function toggleDisplay(){
    setDisplayMode(displayMode === 'art' ? 'visualiser' : 'art');
  }
  ...
  function loadTrack(i, autoplay){
    ...
    const t = tracks[index];
    let url = urls.get(t.file);
    if (!url) { url = URL.createObjectURL(t.file); urls.set(t.file, url); }
    audio.src = url;
    audio.currentTime = 0;
    syncMeta();
  ...
  function toggle(){
    ...
    if (!audio.src) { $('kipFiles').click(); return; }
  ...
  function addFiles(list){
    ...
    files.forEach(f => tracks.push({ file: f, ...readMetadata(f) }));
  ...
  $('kipScreen').addEventListener('click', e => {
    if (e.target.closest('button, .kip-art-wrap, .kip-dolby-row')) return;
    toggleDisplay();
  });
  ...
  $('kipUpload').addEventListener('click', () => $('kipFiles').click());
  $('kipFiles').addEventListener('change', e => { addFiles(e.target.files); e.target.value = ''; });
  ...
  $('kipMenu2').addEventListener('click', () => { $('kipMenu').click(); });

  window.addEventListener('resize', () => { if (displayMode === 'visualiser') resizeCanvas(); });
  ...
  function buildPresets(){
    ...
    groups.forEach(g => {
      ...
      items.forEach(it => {
        ...
        const l = typeof it === 'string' ? it.replace(/^[^-]+[-+]\s*/,'').trim() : it.label;
        html.push(`<button type="button" data-preset="${g.key}::${v}">${l}</button>`);
      });
    });
    $('kipPresetList').innerHTML = html.join('') || '<div style="color:#8a93a3;font-size:11px;padding:6px">No presets loaded yet</div>';
    $('kipPresetList').querySelectorAll('[data-preset]').forEach(b => b.addEventListener('click', () => {
      const [group, ...rest] = b.dataset.preset.split('::');
  ...
  function open(){
    ...
    if (displayMode === 'visualiser') { resizeCanvas(); if (!raf) raf = requestAnimationFrame(drawFrame); }
  }
  function close(){
  ...
  window.kefeIpodPlayer = { version: 3, open, close, addFiles, setDisplayMode };
