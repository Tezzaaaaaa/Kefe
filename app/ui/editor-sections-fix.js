...
(() => {
  ...
  function moveTitleCardIntoExport() {
    ...
    if (!top) {
      ...
      exportSection.insertBefore(top, exportSection.firstChild?.nextSibling || exportSection.firstChild);
    }
  ...
  const bgDefaults = { effect:'none', intensity:.55, speed:1 };
  const clamp = (v,a=0,b=1) => Math.max(a,Math.min(b,Number(v)||0));
  const rand = (n,s=0) => { const x=Math.sin(n*12.9898+s*78.233)*43758.5453; return x-Math.floor(x); };
  ...
  function drawBackgroundEffect(w,h,t,settings){
    ...
    const a=clamp(s.intensity), speed=Math.max(.1,Number(s.speed)||1), time=t*speed;
    bgCtx.clearRect(0,0,w,h);
    if(s.effect==='none') return bgCanvas;
    if(s.effect==='gradient-flow'){
      const x=w*(.5+.38*Math.sin(time*.22)), y=h*(.5+.35*Math.cos(time*.17));
      const g=bgCtx.createRadialGradient(x,y,0,w*.9,h*.7);
      g.addColorStop(0,'rgba(125,82,255,.82)'); g.addColorStop(.38,'rgba(35,84,160,.46)'); g.addColorStop(1,'rgba(3,5,12,1)');
      bgCtx.fillStyle=g; bgCtx.fillRect(0,0,w,h);
    } else if(s.effect==='aurora'){
      bgCtx.fillStyle='#05070c'; bgCtx.fillRect(0,0,w,h);
      for(let i=0;i<6;i++){
        const x=w*(.15+i*.16)+Math.sin(time*.32+i)*w*.12;
        const y=h*(.25+.12*Math.sin(time*.21+i));
        const r=Math.min(w,h)*(.35+.08*Math.sin(time*.3+i));
        const g=bgCtx.createRadialGradient(x,y,0,x,y,r);
        const c=['#5227ff','#36d6c5','#7cff67','#ff4fd8'][i%4];
        g.addColorStop(0,c+'aa'); g.addColorStop(.48,c+'35'); g.addColorStop(1,c+'00');
        bgCtx.fillStyle=g; bgCtx.fillRect(0,0,w,h);
      }
    } else if(s.effect==='waves'){
      bgCtx.fillStyle='#070a10'; bgCtx.fillRect(0,0,w,h);
      for(let i=0;i<9;i++){
        bgCtx.beginPath();
        for(let x=0;x<=w;x+=Math.max(8,w/120)){
          const y=h*(.18+i*.085)+Math.sin(x/w*8+time*(.7+i*.07)+i)*h*(.035+.012*a);
          if(x===0) bgCtx.moveTo(x,y); else bgCtx.lineTo(x,y);
        }
        bgCtx.strokeStyle=`hsla(${190+i*18},80%,${48+i*3}%,${.10+.035*a})`;
        bgCtx.lineWidth=Math.max(2,w*.0025); bgCtx.stroke();
      }
    } else if(s.effect==='particles'){
      bgCtx.fillStyle='#05070b'; bgCtx.fillRect(0,0,w,h);
      const count=Math.round(90+100*a);
      for(let i=0;i<count;i++){
        const x=rand(i,1)*w, base=rand(i,2)*h;
        const y=(base-time*(10+rand(i,3)*26))%h;
        const r=.7+rand(i,4)*2.2;
        bgCtx.globalAlpha=.16+.5*rand(i,5)*a; bgCtx.fillStyle='#ffffff'; bgCtx.beginPath(); bgCtx.arc(x,y,r,0,Math.PI*2); bgCtx.fill();
      }
      bgCtx.globalAlpha=1;
    } else if(s.effect==='spotlight-pulse'){
      bgCtx.fillStyle='#070504'; bgCtx.fillRect(0,0,w,h);
      const pulse=.55+.45*Math.sin(time*1.6);
      const g=bgCtx.createRadialGradient(w*.5,h*(.38+.025*pulse),0,w*.5,h*.4,Math.max(w,h)*(.55+.08*pulse));
      g.addColorStop(0,`rgba(255,205,130,${.72*a})`); g.addColorStop(.34,`rgba(125,72,32,${.32*a})`); g.addColorStop(1,'rgba(0,0,0,1)');
      bgCtx.fillStyle=g; bgCtx.fillRect(0,0,w,h);
    } else if(s.effect==='film'){
      bgCtx.fillStyle='#100d0a'; bgCtx.fillRect(0,0,w,h);
      const count=Math.round(w*h*.000018*(.5+a));
      for(let i=0;i<count;i++){
        const x=Math.floor(rand(i,time*7)*w), y=Math.floor(rand(i,time*11)*h), v=rand(i,time*13)>.5?255:0;
        bgCtx.fillStyle=`rgba(${v},${v},${v},${.05+.12*a})`; bgCtx.fillRect(x,y,1,1);
      }
    }
    bgCtx.globalAlpha=1;
    return bgCanvas;
  ...
  function installBackgroundRenderer(){
    ...
    window.render=function(ctx,w,h,appState,mediaCache){
      ...
      try { original(ctx,w,h,appState,mediaCache); }
      finally {
  ...
  function addBackgroundEffects(){
    ...
    defs.forEach(([name,label,desc])=>{
      const b=document.createElement('button');b.type='button';b.className='background-fx-button';b.dataset.backgroundFx=name;
      ...
      b.addEventListener('click',()=>{
        ...
        grid.querySelectorAll('.background-fx-button').forEach(x=>x.classList.toggle('active-background-fx',x.dataset.backgroundFx===name));
        window.redrawCurrentPreviewFrame?.();
      });
      grid.appendChild(b);
    });
    const controls=block.querySelector('.background-effects-controls');
    controls.innerHTML='<div class="control-row"><label for="backgroundFxIntensity">Intensity <span id="backgroundFxIntensityVal">55%</span></label><input id="backgroundFxIntensity" type="range" min="0" max="100" step="1" value="55"></div><div class="control-row"><label for="backgroundFxSpeed">Animation speed <span id="backgroundFxSpeedVal">1.0×</span></label><input id="backgroundFxSpeed" type="range" min="25" max="250" step="5" value="100"></div>';
    const intensity=controls.querySelector('#backgroundFxIntensity'), speed=controls.querySelector('#backgroundFxSpeed');
    intensity.addEventListener('input',()=>{window.state.background.intensity=Number(intensity.value)/100;controls.querySelector('#backgroundFxIntensityVal').textContent=`${intensity.value}%`;window.redrawCurrentPreviewFrame?.();});
    speed.addEventListener('input',()=>{window.state.background.speed=Number(speed.value)/100;controls.querySelector('#backgroundFxSpeedVal').textContent=`${(Number(speed.value)/100).toFixed(1)}×`;window.redrawCurrentPreviewFrame?.();});
    section.appendChild(block);
    const current=window.state.background.effect||'none';
    grid.querySelectorAll('.background-fx-button').forEach(x=>x.classList.toggle('active-background-fx',x.dataset.backgroundFx===current));
  }
