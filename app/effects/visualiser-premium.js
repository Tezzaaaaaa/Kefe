...
(function () {
  ...
  function ferrofluid(ctx, w, h, time, frame) {
    ...
    ctx.fillRect(0,0,w,h);
    ctx.globalCompositeOperation = 'lighter';
    var n = 96, R = min * (.16 + a.b * .055), pulse = 1 + a.b * .22;
    for (var i=0;i<n;i++) {
      var ang = TAU*i/n + time*.08;
      var wave = .72 + .28*Math.sin(i*2.37 + time*1.4 + a.m*4);
      ...
      grad.addColorStop(.82,'rgba(255,255,255,' + (.18+a.t*.45).toFixed(3) + ')');
      grad.addColorStop(1,'rgba(255,255,255,0)');
      ctx.strokeStyle=grad; ctx.lineWidth=Math.max(1,min*(.004+a.t*.006));
  ...
  function liquidGlass(ctx,w,h,time,frame){
    var a=audio(frame), cx=w*.5,cy=h*.5,min=Math.min(w,h);
    ctx.save(); ctx.globalCompositeOperation='source-over';
    ctx.fillStyle='rgba(1,2,8,.16)';ctx.fillRect(0,0,w,h);
    ctx.globalCompositeOperation='screen';
    for(var k=0;k<9;k++){
      var ang=time*(.05+k*.006)+k*.71, rx=min*(.18+k*.025+a.b*.045), ry=rx*(.42+.12*Math.sin(time*.3+k));
      ctx.save();ctx.translate(cx,cy);ctx.rotate(ang);
      ctx.strokeStyle='rgba(225,235,255,'+(.08+a.t*.05)+')';ctx.lineWidth=min*(.012+a.e*.006);
      ctx.beginPath();
    ...
    g.addColorStop(0,'rgba(255,255,255,'+(.22+a.t*.18)+')');g.addColorStop(.45,'rgba(145,190,255,.09)');g.addColorStop(1,'rgba(0,0,0,0)');
  ...
  function cinematicFluid(ctx,w,h,time,frame){
    var a=audio(frame);
    ...
    for(var i=0;i<fluid.length;i++){
      ...
      var ang=Math.sin(p.x*7+time*.45)+Math.cos(p.y*9-time*.31);
      ang*=1.7+a.m*3;
      p.vx+=(Math.cos(ang)*.0007 + (w*.5-px)/w*.00005*a.b);
      p.vy+=(Math.sin(ang)*.0007 + (h*.5-py)/h*.00005*a.b);
      p.vx*=.985;p.vy*=.985;p.x=(p.x+p.vx+1)%1;p.y=(p.y+p.vy+1)%1;
      var r=2+a.t*4;
      ctx.fillStyle=rgb(.56+p.h*.18,.55,.45+a.e*.5,.12+a.e*.12);
  ...
  /* 4 — FRACTAL PLANET */
  function fractalPlanet(ctx,w,h,time,frame){
    ...
    ctx.strokeStyle='rgba(180,215,255,'+(.16+a.t*.25)+')';ctx.lineWidth=min*.004;ctx.beginPath();ctx.arc(cx,cy,R*1.15,0,TAU);ctx.stroke();ctx.restore();
  }

  /* 5 — COSMIC ATTRACTOR */
  var attract=Array.from({length:700},function(_,i){return {x:Math.cos(i)*.5,y:Math.sin(i*1.7)*.5,z:hash(i)*.5};});
  function cosmicAttractor(ctx,w,h,time,frame){
    var a=audio(frame),cx=w*.5,cy=h*.5,min=Math.min(w,h);
    ...
    for(var i=0;i<attract.length;i++){
      var p=attract[i], dx=Math.sin(p.y*2.4+time*.32)*.004*(1+a.b), dy=Math.sin(p.x*1.7-time*.21)*.004*(1+a.m), dz=Math.cos(p.x+p.y+time*.18)*.003;
      ...
      ctx.fillStyle=rgb(.58+p.z*.1,.7,.55+a.t*.4,.18+a.e*.35);ctx.beginPath();ctx.arc(px,py,1+s*2.2,0,TAU);ctx.fill();
    }
    ctx.restore();
  }

  /* 6 — GYROID CRYSTAL */
  function gyroidCrystal(ctx,w,h,time,frame){
    var a=audio(frame),cx=w*.5,cy=h*.5,min=Math.min(w,h);
    ...
    for(var layer=0;layer<34;layer++){
      var rr=min*(.07+layer*.007)*(1+a.b*.18), rot=time*.15+layer*.17;
      ctx.save();ctx.translate(cx,cy);ctx.rotate(rot);
      var pts=28;
      ctx.beginPath();
      for(var i=0;i<=pts;i++){var q=i/pts*TAU;var gy=Math.sin(q*3+time*.6)+Math.sin(q*5-time*.3);var rad=rr*(1+.16*gy+a.m*.06*Math.sin(q*11));var x=Math.cos(q)*rad,y=Math.sin(q)*rad;i?ctx.lineTo(x,y):ctx.moveTo(x,y);}
      ctx.strokeStyle=rgb(.54+layer*.004,.3,.65+a.t*.25,.025+a.t*.015);ctx.lineWidth=min*(.0015+a.e*.001);ctx.stroke();ctx.restore();
    }
    ctx.restore();
  }

  /* 7 — SONIC METABALL ORGANISM */
  function metaball(ctx,w,h,time,frame){
    var a=audio(frame),cx=w*.5,cy=h*.5,min=Math.min(w,h),N=7;
    ctx.save();ctx.globalCompositeOperation='source-over';ctx.fillStyle='rgba(0,0,0,.12)';ctx.fillRect(0,0,w,h);ctx.globalCompositeOperation='lighter';
    for(var i=0;i<N;i++){
      var q=TAU*i/N+time*.25*(.3+a.e), rr=min*(.11+a.b*.055), x=cx+Math.cos(q*1.7)*min*(.16+a.m*.05), y=cy+Math.sin(q*1.3)*min*(.16+a.b*.04);
      var g=ctx.createRadialGradient(x-rr*.28,y-rr*.32,0,x,y,rr*1.25);
      g.addColorStop(0,'rgba(255,255,255,'+(.55+a.t*.35)+')');g.addColorStop(.38,'rgba(145,175,205,.28)');g.addColorStop(.78,'rgba(40,50,65,.15)');g.addColorStop(1,'rgba(0,0,0,0)');
      ctx.fillStyle=g;ctx.beginPath();ctx.arc(x,y,rr*(1+.18*Math.sin(time*1.2+i)+a.b*.12),0,TAU);ctx.fill();
    }
    ctx.restore();
  }

  /* 8 — HOLOGRAPHIC RIBBON SCULPTURE */
  function ribbon(ctx,w,h,time,frame){
    ...
    for(var band=0;band<7;band++){
      ctx.beginPath();
      for(var i=0;i<=160;i++){var u=i/160, x=u*w, y=h*.5+Math.sin(u*TAU*(1.2+band*.13)+time*(.3+band*.04))*h*(.13+band*.012)+Math.sin(u*TAU*3-time)*h*.035*a.b; y+=(band-3)*min*.035; i?ctx.lineTo(x,y):ctx.moveTo(x,y);}
      ctx.strokeStyle=rgb(.56+band*.025,.65,.65+a.t*.3,.12+a.e*.08);ctx.lineWidth=min*(.008+a.b*.018);ctx.stroke();
    }
    ctx.restore();
  }

  /* 9 — BLACK-HOLE ACCRETION */
  function blackHole(ctx,w,h,time,frame){
    var a=audio(frame),cx=w*.5,cy=h*.5,min=Math.min(w,h);
    ...
    for(var i=0;i<420;i++){
      var r=min*(.10+hash(i)*.42)*(1+a.b*.1), ang=hash(i+3)*TAU+time*(.12+.5*(1-r/min))*(1+a.e);
      var x=cx+Math.cos(ang)*r,y=cy+Math.sin(ang)*r*.34;
      var z=hash(i+8);ctx.fillStyle=rgb(.55+z*.12,.85,.45+a.t*.4,.12+a.e*.18);
      ctx.beginPath();ctx.arc(x,y,1+z*2.4+a.t*1.5,0,TAU);ctx.fill();
    }
    var g=ctx.createRadialGradient(cx,cy,min*.045,cx,cy,min*.18);
    g.addColorStop(0,'rgba(0,0,0,1)');g.addColorStop(.5,'rgba(0,0,0,.98)');g.addColorStop(.72,'rgba(255,180,70,'+(.18+a.b*.3)+')');g.addColorStop(1,'rgba(0,0,0,0)');
  ...
  function neural(ctx,w,h,time,frame){
    var a=audio(frame);
    ctx.save();ctx.globalCompositeOperation='source-over';ctx.fillStyle='rgba(0,0,0,.1)';ctx.fillRect(0,0,w,h);ctx.globalCompositeOperation='lighter';
    for(var i=0;i<nodes.length;i++){var n=nodes[i];n.p+=.004*(1+a.e*4);n.x+=Math.sin(n.p)*.00035*(.4+a.m);n.y+=Math.cos(n.p*1.17)*.00035*(.4+a.b);n.x=(n.x+1)%1;n.y=(n.y+1)%1;}
    for(var i=0;i<nodes.length;i++)for(var j=i+1;j<nodes.length;j++){var p=nodes[i],q=nodes[j],dx=p.x-q.x,dy=p.y-q.y,d=Math.sqrt(dx*dx+dy*dy);if(d<.19){var alpha=(.19-d)/.19*(.06+a.e*.16);ctx.strokeStyle=rgb(.56,.55,.65,alpha);ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(p.x*w,p.y*h);ctx.lineTo(q.x*w,q.y*h);ctx.stroke();}}
    for(var k=0;k<nodes.length;k++){var p=nodes[k],r=1.5+p.v*2+a.b*4;ctx.fillStyle=rgb(.57+p.v*.08,.7,.7+a.t*.3,.28+a.e*.35);ctx.beginPath();ctx.arc(p.x*w,p.y*h,r,0,TAU);ctx.fill();}
    ctx.restore();
  }
