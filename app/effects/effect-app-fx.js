...
(() => {
    ...
    function ui(){const host=document.getElementById('effectControls');if(!host||host.dataset.kefeFxBuilt)return;host.dataset.kefeFxBuilt='1';
      const filterBlock=document.createElement('div');filterBlock.className='sub-block';
      const filterHeading=document.createElement('div');filterHeading.className='sub-heading';filterHeading.textContent='Filters';filterBlock.appendChild(filterHeading);
      const filterButtons=document.createElement('div');filterButtons.className='effect-buttons';filterButtons.id='visualFxFilterButtons';
      ['none','vhs','crt','rgb','bloom','motion','shake','glitch','halftone','vignette','mixedmedia'].forEach(n=>{const b=document.createElement('button');b.type='button';b.dataset.fx=n;b.className='kefe-fx-button';b.textContent=n==='none'?'Off':n==='rgb'?'RGB':n==='mixedmedia'?'Mixed Media':n[0].toUpperCase()+n.slice(1);b.addEventListener('click',()=>setFx(n));filterButtons.appendChild(b);});
      filterBlock.appendChild(filterButtons);
      host.appendChild(filterBlock);
      const bgBlock=document.createElement('div');bgBlock.className='sub-block';
      const bgHeading=document.createElement('div');bgHeading.className='sub-heading';bgHeading.textContent='Backgrounds';bgBlock.appendChild(bgHeading);
      const bgButtons=document.createElement('div');bgButtons.className='effect-buttons';bgButtons.id='visualFxBackgroundButtons';bgBlock.appendChild(bgButtons);
      host.appendChild(bgBlock);
      const d=document.createElement('div');d.className='effect-label';d.id='visualFxLabel';d.textContent=labels[window.state.style.visualFx]||labels.none;host.appendChild(d);
      const controls=document.createElement('div');range(controls,'fxIntensity','Intensity',0,1,.05);range(controls,'fxSpeed','Animation speed',.25,2.5,.05,'×');host.appendChild(controls);
