const APPLE_FONT_STACK = window.KEFE_APPLE_FONT_STACK || '"SF Pro Display","SF Pro Text",-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif';
...
function drawAppleEffect(ctx, w, h, style, lines, time) {
    ...
    const lyricLines = source.map(function(line) {
        const words = Array.isArray(line.words) ? line.words.map(function(word) {
            return {
                startTime: Number(word.time) * 1000,
                endTime: Number(word.endTime) * 1000,
                word: String(word.text != null ? word.text : word.word || ''),
                isBG: !!word.background
            };
        }) : [];
        return {
            words: words,
            startTime: Number(line.time) * 1000,
            endTime: Number(line.endTime) * 1000,
            isBG: !!line.background,
            isDuet: !!line.isDuet,
            translatedLyric: String(line.translation || line.translatedLyric || ''),
            romanLyric: String(line.transliteration || line.romanLyric || ''),
            text: String(line.text || '')
        };
    });

    const renderer = window.kefeAppleLyricsEffect;
    ...
    renderer.render(
        ...
        Number(time) * 1000,
        {
            ...
            lineHeight: 1.25,
            lineSpacing: 22 * (Number(style.fxSpacing) || 1),
            activeOpacity: 1,
            inactiveOpacity: Number.isFinite(Number(style.appleInactiveOpacity)) ? Number(style.appleInactiveOpacity) : 0.32,
            pastOpacity: 0.14,
            activeScale: 1,
            inactiveScale: 0.985,
            blurRadius: 1.2,
            ...
            paddingTop: Number(style.appleTopOffset) || 0.245,
            visibleLines: Math.max(3, Math.min(7, Math.round(Number(style.appleVisibleLines) || 4))),
